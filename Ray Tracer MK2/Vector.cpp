...
float Vector::dot(const Vector& other) {
	...
	return _mm_cvtss_f32(_mm_dp_ps(this->sseVector, other.sseVector, 0x71));
}

float Vector::magnitudeSquared() {
	//return this->x * this->x + this->y * this->y + this->z * this->z;

	return _mm_cvtss_f32(_mm_dp_ps(this->sseVector, this->sseVector, 0x71));
}

float Vector::magnitude() {
	//return sqrt(this->magnitudeSquared());  // Seems to be slower

	return _mm_cvtss_f32(_mm_sqrt_ss(_mm_dp_ps(this->sseVector, this->sseVector, 0x71)));
}
...
Vector Vector::normalized() {
	...
	__m128 inverseSqrt = _mm_rsqrt_ps(_mm_dp_ps(this->sseVector, this->sseVector, 0x77));
